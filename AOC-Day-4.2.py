...
answer = 0
def term_check(line):
    terms = ['pid','hcl','hgt','ecl','byr','eyr','iyr']
    if (all(i in line for i in terms)) == True:
        return True
    else:
        return False
...
def validator(password):
    ...
    try:
        byr = re.findall(r'byr:[0-9]{4}', password)
    ...
    try:
        hgt = re.findall(r'hgt:\w+', password)
    ...
    try:
        hcl = re.findall(r'hcl:#[0-9a-f]{6}\W', password)
    ...
    try:
        ecl = re.findall(r'ecl:\D{3}', password)
    except:
    ...
    try:
        pid = re.findall(r'pid:\d{9}\W', password)
...
while count <= total - 1:
    if term_check(sep_passwords[count]) == True:
        if validator(sep_passwords[count]):
